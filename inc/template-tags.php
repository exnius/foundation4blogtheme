...
function foundation4blogtheme_comment( $comment, $args, $depth ) {
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
		<div class="comment-body">
			<?php _e( 'Pingback:', 'foundation4blogtheme' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'foundation4blogtheme' ), '<span class="edit-link">', '</span>' ); ?>
		</div>

	<?php else : global $post; ?>
		...
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
			<footer class="comment-meta vcard">
				...
				<div class="comment-metadata">
					...
					<?php
					printf( __( '%s <span class="says">says:</span>', 'foundation4blogtheme' ),
						sprintf( '<cite class="fn">%1$s %2$s</cite>',
							get_comment_author_link(),
							( $comment->user_id === $post->post_author ) ? '<span class="label"> ' . __( 'Post author', 'foundation4blogtheme' ) . '</span>' : ''
						 )
					);
					?>
					</div>
					...
					<?php edit_comment_link( __( 'Edit', 'foundation4blogtheme' ), '<span class="sep"> | </span><span class="edit-link">', '</span>' ); ?>
					</div>
			...
			<div class="comment-content">
				...
				<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'foundation4blogtheme' ); ?></p>
				<?php endif; ?>
...
if ( ! function_exists( 'oundation4blogtheme_the_attached_image' ) ) :
/**
 * Prints the attached image with a link to the next attached image.
 */
function foundation4blogtheme_the_attached_image() {
	$post                = get_post();
	$attachment_size     = apply_filters( 'foundation4blogtheme_attachment_size', array( 1200, 1200 ) );
